if(typeof jQuery == 'undefined')
	...
	(function(filten, $, undefined) {
		filten.options = {};
		
		filten.FiltenVM = function() {
			var self = this;
			...
			$('.filten-item').each(function (i, v) {
				...
				$.each(item.criteria, function (criterionKey, rawValue) {
					...
					if(!criteriaMap[criterionKey])
						criteriaMap[criterionKey] = new CriterionModel(criterionKey, filten.options[criterionKey.toLowerCase()], self.valueSelectionCB);
	...
	}(window.filten = window.filten || {}, jQuery));
	...
	$(function(){
		$('.filten').each(function(){
			...
			ko.applyBindings(new filten.FiltenVM(), this);
			$filten.show();
		});
	});
}